...
graphcorrelation <- function(Peak_annotation,PeakCount,TranscriptCount,ProteinCount,orderSamples,graphSamples){
  ...
  for(p in 1:nrow(Peak_annotation)){
    ...
    peakName <- unlist(unname(Peak_annotation[p,2]))
    ...
    peak_count <- unlist(unname(dplyr::filter(PeakCount,Gene==gene,peakName==peakName)[1,3:ncol(PeakCount)]))
    ...
    PeakGene_xor_values <- dplyr::bind_rows(PeakGene_xor_values,dplyr::tibble(Gene=gene,PeakName=peakName,Status=status,TranscriptPeakXor=trans_peak_xor,TranscriptProteinXor=trans_prot_xor))
  }
  PeakGene_xor_values
}
